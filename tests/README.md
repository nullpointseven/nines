...
| `install-args` | shell test | `scripts/install-lib.sh`: argument parsing (host/device/flags matrix, `INSTALL_HOST`), `disko_disks_json` validation/rendering, and the interactive deus-vault disk-selection prompt (variable RAID member count) + `DEUS_VAULT_DISKS` handling |
